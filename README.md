...
```
setup.py                          - classic setuptools shim (name/version/packages), kept alongside
                                     pyproject.toml specifically so Frappe Cloud's app validator, which
                                     does not reliably parse pyproject.toml-only apps, can detect the
                                     app name and package list
requirements.txt                  - Python dependencies (mirrors pyproject.toml's dependencies)
bog_connector/                    - the app package (repo_root/bog_connector/hooks.py)
